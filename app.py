...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .votes { font-size: 3rem; font-weight: bold; color: #667eea; margin: 1rem 0; }
        .btn { background: linear-gradient(135deg, #667eea, #764ba2); color: #fff; border: none; padding: 0.75rem 1.5rem; border-radius: 8px; cursor: pointer; font-size: 1rem; transition: opacity 0.2s; }
...
<body>
    ...
    <script>
        ...
        async function loadVotes() {
            ...
            grid.innerHTML = '';
            data.forEach(r => {
                grid.innerHTML += `<div class="card"><h2>${r.name}</h2><div class="votes">${r.value}</div><button class="btn" onclick="vote('${r.name}')">Vote</button></div>`;
            });
        }
